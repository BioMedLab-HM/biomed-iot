...
The setup_files/logs subfolder stores log files generated by both this main 
setup script and the individual installation scripts. These logs provide 
valuable insights into the installation process and can help troubleshoot any 
issues that arise.
"""
...
def get_credentials_for_pw_reset():
    ...
    while True:
        ...
        if user_answer == 'y':
            pwreset_host = get_confirmed_text_input("Enter the host "
                "for the website's password reset function (e.g. smtp.gmail.com)"
                )
            pwreset_port = int(get_confirmed_text_input("Enter the port "
...
def main():
    ...
    if not auto_generate_django_admin_credentials:
        django_admin_email = get_confirmed_text_input("Enter a username for "
            "your website's admin user"
        )
    ...
    pw_reset_credentials = {"RES_EMAIL_HOST":pwreset_host,
